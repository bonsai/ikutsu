...
## [Unreleased]
### Added
- ランディング説明：「#生成AIなんでも展示会」・「エージェントがあなたの年齢当ててみせるわ」
- 「写真は保存されません」明記
- 結果画面から「コピー」ボタン削除
- 10秒経過で自動的に最初に戻る（もう一度ボタン不要）
- Twitterシェアフォーマット更新（#VonsaiApps タグ）
- 切り替えボタン削除（シンプルに）
- ページロード時の自動カメラ起動をやめ、ボタンから起動
- 写真保存なし（スクリーンショット機能削除）
- 新应用「kidoairaku」（喜怒哀楽）发布
- kidoairaku: 楽の表示を汉字「楽」に変更
- フィードバック機能：だいたいあってる✅／ぜんぜんちがう❌
- モデル評価統計ページ `/stats` 公開（🎉 黄色テーマ）
- 評価データ CSV 保存（evaluations.csv）
- ボタン「始める」に表記統一
